...
def plot_accuracy_by_position(results: dict, output_dir: Path):
    ...
    for i, (model_name, model_results) in enumerate(results["models"].items()):
        accuracies = [
            model_results["positions"][str(p)]["accuracy"] * 100
            for p in positions
        ]
...
def print_summary_table(results: dict):
    ...
    # Header
    header = "Model".ljust(15) + " | " + " | ".join([f"Pos {p:>2}" for p in positions]) + " | Drop"
    print(header)
    print("-" * len(header))
...
def main():
    ...
    parser.add_argument("--input", "-i", type=str, default="results/results_v2.json",
